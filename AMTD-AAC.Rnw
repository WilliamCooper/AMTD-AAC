...
PlotsAtEnd <- TRUE    ## if FALSE, plots are in-line in text
ReloadData <- FALSE
# ReloadData <- TRUE  ## used to retrieve original data rather than saved .Rdata files
...
thisFileName <- "AMTD-AAC"
setwd("~/RStudio/AMTD-AAC")
Directory <- DataDirectory ()
Flight <- "ff02" 		# second ferry flight
Project = "DEEPWAVE"	
...
## these functions are used to displace plots to the end, if PlotsAtEnd is TRUE
SavePlot <- function (.figname) {
  if (PlotsAtEnd) {
    cairo_pdf (sprintf ("./Graphics/%s.pdf", .figname))
    dev.control ("enable")
  }
}
EndSave <- function () {
...
## these figure captions are used for the figure-caption list and also are attached to the figures
...
## this generates a plot of velocity and pitch/roll errors in the l-frame
SavePlot ("Fig1")
...
## the plot generated here is not included in the manuscript draft -- "include=FALSE" above
Flight <- "rf01h"
Flight <- "rf01"
...
D <- Data[Data$TASX > 130, ]    ## avoid possible periods with flaps
...
## calculate derivatives -- note need for "Rate" here (but Rate = 1, this file)
# 1013 points (must be odd) to span about 1/5 Schuler osc. -- about 16.8 min
NAV <- 1013*Rate
vndot <- signal::sgolayfilt (VNS-GGVNS, 3, NAV, m=1) * Rate  # m=1 for first deriv.
vedot <- signal::sgolayfilt (VEW-GGVEW, 3, NAV, m=1) * Rate
...
## add second panel with the a-frame errors
op <- par (mar=c(5,4,1,2.5)+0.1)
...
## get data for next flight to use
Flight <- "rf15" 				
Project = "DEEPWAVE"			 
...
## this function applies the rotation correction to the accelerations after transformation to the l-frame
...
## get l-frame accelerations
G <- Gravity (D$LATC, D$GGALT)    ## Ranadu gravity function, approx. matching that of the INS
...
D$LACCY <- LV[, 2]
D$LACCZ <- LV[, 3] + G
## apply averaging to match that used for GPS values, exc. use smaller value for
## the z component because the vertical acceleration varies rapidly in the pitch maneuver
...
r <- setRange(D$Time, 33500,35500)  ## select the period of a circle maneuver
...
## plot error estimates "herr"
SavePlot ("Fig4")
## still using the data.frame constructed earlier, D
A2 <- D$LACCX^2 + D$LACCY^2
A <- sqrt(A2)
## error estimate from the "estimator-equation" (10)
D$herr <- (-D$LACCY*(D$vedot-D$LACCX)+D$LACCX*(D$vndot-D$LACCY)) / (Cradeg*A2)
#plotWAC(D$Time, D$herr, ylim=c(-2,2))

## qualify the points, but don't change D in case it is still needed
DT <- D
v <- DT$TASX > 130
...
## put the average value of the heading correction into D
# D$hs <- rep(NA, nrow(DT))
D$hs <- rep(meanDh, nrow(DT))
## replace the mean value with the error estimate where valid
for (i in 1:nrow(ts)) {
  ix <- which(D$Time == ts$Time[i])
  D$hs[ix] <- ts$herr[i]
}
## interpolate, filter, plot
...
## superfluous: this file is 1-Hz
  ...
  ## form new data.frame for generating plot
  ...
  D1$HC <- predict(SS, as.numeric(D1$Time))$y  ## this uses the fitted spline SS
  if (GeneratePlot) {
...
## load data for next flight and apply time shifts and pitch/roll correction

Flight <- "rf16h"   ## 25-Hz file
fname <- sprintf ("%s%s/%s%s.nc", DataDirectory(), Project, Project, Flight)
...
## construct and work with 1-Hz data.frame, for processing speed
if (Rate > 1) {
  D1 <- D[(as.numeric(D$Time) %% 1) < 0.01,]
} else {
  D1 <- D
...
## this would construct a plot like the first one for flight 15, but is now suppressed
A2 <- D1$LACCX^2 + D1$LACCY^2
A <- sqrt(A2)
D1$herr <- (-D1$LACCY*(vedot-D1$LACCX)+D1$LACCX*(vndot-D1$LACCY)) / (Cradeg*A2)
# plotWAC(D$Time, D$herr, ylim=c(-2,2))
...
## this chunk constructs the plot of mean values from turns and the spline fit, for Flt 16
SavePlot ("Fig6")
## this function was used in development; not needed now
...
# D$HC <- predict(SS, as.numeric(D$Time))$y  ## not used at high rate
...
## plot density function showing magnitude of pitch corrections
SavePlot ("Fig7")
# Project <- "DEEPWAVE"
# Flight <- "rf16"
...
#                    "LATC", "GGALT"))
## not necessary to load data.frame; still have 1-Hz file from prev. section
DD <- D1
...
legend('topright', legend='uncorrected', inset=0.1, adj=-0.05, bty="n", box.col='white', text.col='blue')
op <- par (mar=c(5,4,1,2.5)+0.1)
...
legend('topright', legend="after correction", inset=0.1, adj=-0.05, bty="n", box.col='white', text.col='red')
...
in relatively steady wind conditions.\label{tab:Flight-segments}}
...
\begin{table*}
\begin{raggedright}
\caption[\foreignlanguage{british}{Elemental contributions to the uncertainty in measurement of vertical
...
static or ambient pressure, and ATX is the temperature measurement.}\selectlanguage{british}%
 ...
 \vfill\eject\textbf{Table 2}:\vskip0.1truein\textbf{}\\
...
\begin{table*}
\raggedright{}\caption[\foreignlanguage{british}{Elemental contributions to the uncertainty in measurement of horizontal
...
wind, and those with subscript $\mkern3mu\protect\vphantom{\perp}\vrule depth0pt\mkern3mu\vrule depth0pt\mkern3mu$
refer to the longitudinal component (along the axis of the aircraft).
All entries apply to 1-Hz measurements.}\label{tab:Elemental-h}\foreignlanguage{british}{The
...
further information on the elements in this table and for explanation
of some of the remaining terms.}\selectlanguage{british}%
}
\vfill\eject\textbf{Table 3}:\vskip0.1truein\textbf{}\\
\textbf{}%
\begin{minipage}[t]{1\columnwidth}%
\begin{flushleft}
 %
\begin{tabular}{ccccccc}
\toprule 
\textbf{element}  & \textbf{uncertainty}  & \textbf{bias}  & \textbf{random}  & \textbf{DOF}  & $\delta u_{\perp,\,\mkern3mu\vphantom{\perp}\vrule depth0pt\mkern3mu\vrule depth0pt\mkern3mu}$
\textbf{bias}  & $\delta u_{\perp,\,\mkern3mu\vphantom{\perp}\vrule depth0pt\mkern3mu\vrule depth0pt\mkern3mu}$
\textbf{random }\tabularnewline
...
\begin{description}
\item [{Figure~1:}] \Sexpr{FigCap1}
\item [{Figure~2:}] \Sexpr{FigCap2}
\item [{Figure~3:}] \Sexpr{FigCap3}
\item [{Figure~4:}] \Sexpr{FigCap4}
\item [{Figure~5:}] \Sexpr{FigCap5}
\item [{Figure~6:}] \Sexpr{FigCap6}
\item [{Figure~7:}] \Sexpr{FigCap7}
\item [{Figure~8:}] \Sexpr{FigCap8}
\end{description}
\begin{center}
\begin{figure}
\begin{centering}
\includegraphics[height=0.65\textheight]{figure/v-errors-straight-leg-1}
\par\end{centering}

\caption{\Sexpr{FigCap1}\label{fig:v-errors-straight-leg}}
\end{figure}
...
\begin{figure}
\begin{centering}
\includegraphics[height=0.65\textheight]{figure/processing-2-1}
\par\end{centering}

\caption{\Sexpr{FigCap2}\label{fig:processing-2}}
\end{figure}
\begin{figure}
\begin{centering}
\includegraphics[height=0.65\textheight]{figure/get-b-vector-and-transform-1}
\par\end{centering}

\caption{\Sexpr{FigCap3}\label{fig:get-b-vector-and-transform}}
\end{figure}
\begin{figure}
\begin{centering}
\includegraphics[height=0.65\textheight]{figure/error-components-1}
\par\end{centering}

\caption{\Sexpr{FigCap4}\label{fig:error-components}}
\end{figure}
\begin{figure}
\begin{centering}
\includegraphics[height=0.65\textheight]{figure/plot-heading-correction-rf15-1}
\par\end{centering}

\caption{\Sexpr{FigCap5}\label{fig:plot-heading-correction-rf15}}
\end{figure}
\begin{figure}
\begin{centering}
\includegraphics[height=0.65\textheight]{figure/spline-plot-1}
\par\end{centering}

\caption{\Sexpr{FigCap6}\label{fig:spline-plot}}
\end{figure}
\begin{figure}
\begin{centering}
\includegraphics[height=0.65\textheight]{figure/typical-errors-1}
\par\end{centering}

\caption{\Sexpr{FigCap7}\label{fig:typical-errors}}
\end{figure}
\begin{figure}
\begin{centering}
\includegraphics[width=0.8\textwidth]{figure/reverse-course-w-comparison-1}
\par\end{centering}

\caption{\Sexpr{FigCap8}\label{fig:reverse-course-w-comparison}}
\end{figure}
